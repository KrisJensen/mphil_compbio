...
function interaction_time2(state1, state2, tstar, a0; Print = false)
    ...
    for i in 1:9
        ...
        if (B^2 - 4*A*C) > 0
            ...
            for deltat in [deltat2]#[deltat1, deltat2]
...
function initialize(n, xlo, xhi, ylo, yhi, Print = false)
    ...
    for i = 1:n
        ...
        state = vcat( state, hcat([ vcat(r, v) ], [ vcat(r, v) ] ) )
...
function growing_disks(;n=2, a0 = 1, xlo=0, xhi=xhi, ylo=0, yhi=yhi, end_time=1000000, Print = true, maxiter = 10)
    ...
    while (current_time <= end_time) & (niter < maxiter)
        ...
        for i in 1:n #update old times
            state[i,Old[i]] = move_to_grid( advance(state[i,Old[i]], current_time - time[i, Old[i]]),xlo,xhi,ylo,yhi)
        ...
        collision = true; tmin = Inf; indj=NaN; kmin=NaN; sectind = NaN
        for j in indi_bar
            ...
            for ind in 1:length(tcols)
                ...
                if tcol < tmin && newtimes[j] > tcol
                    add = true
                    for prev in 1:50
                        if indi == recent_events[prev,1] && j == recent_events[prev,2] && abs(tcol - recent_events[prev,3]) < 0.0001
                    ...
                    if add
                        ...
                        indj = j
                        sectind = sectorind
                        Print && println("new tmin ", tmin, " collide ", indi, " with ", indj, " sector: ", sectind)
                    end
                end
            end
        ...
        for k in 1:4
            ...
            if tcross < tmin
                add = true
                for prev in 1:50
                    if indi == recent_events[prev,1] && isnan(recent_events[prev,2]) && abs(tcross - recent_events[prev,3]) < 0.000001
                        add = false #already processed this event
                        Print && println("might have processed this event? ", recent_events[prev,:])
                    end
                end
        ...
        if tmin < Inf
            ...
            if !collision
                state[indi, Int(New[indi])] = jump1(state1, kmin)
                ...
                dist = norm(state2[1:2]-(state1[1:2]+sects[sectind]))
                if round(dist) != round(a0*tmin)
                    println("ERROR! dists do not match. Dist: ", dist, "  a(t): ", a0*tmin)
                end
                Print && println("Distance: ", dist, "  a=", a0*tmin)
                ...
                if (!isnan(indm)) & (indm != indi) #j has collided with i, is no longer ms next partner
                    ...
                    state[indm, New[indm]] = advance(state[indm, New[indm]],
                                                    time[indm, New[indm]] - current_time)
